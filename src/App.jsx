...
export default function App() {
  ...
  // Chat hanya untuk tanya-jawab manual via claude.ai artifact API
  async function handleChat() {
    const text = manual.trim();
    if (!text || chatLoad) return;
    const ctx = decision ? `Konteks keputusan: ${decision.signal} ${decision.pair} | Entry: ${decision.setup?.entry||"-"} | SL: ${decision.setup?.sl||"-"} | TP: ${decision.setup?.tp||"-"}` : "";
    setChatMsgs(prev=>[...prev,{role:"user",content:text}]);
    setManual("");
    const nh = [...chatHistory, {role:"user", content: ctx ? `${ctx}\n\n${text}` : text}];
    setChatHistory(nh);
    setChatLoad(true);
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,
          system:"Kamu adalah asisten trading yang membantu trader memahami analisis MA13/21, VuManChu, dan S&R. Jawab singkat dan praktis dalam Bahasa Indonesia.",
          messages:nh}),
      });
      ...
      setChatMsgs(prev=>[...prev,{role:"assistant",content:"Gagal terhubung."}]);
